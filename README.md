...
- **Сверка ставок** — команда, которая проверяет, сходятся ли наши цены с тем, что считает сам Claude Code, и предлагает поправку, если у вашего плана свои ставки. Она же дописывает ставки новой модели, если панель заметила такую в журналах.
...
Когда выходит новая модель, её ставок в таблице ещё нет, и её запросы шли бы в суммы по $0. Поэтому панель показывает в шапке красную строку — какая модель и сколько её запросов без цены, — пока ставки не записаны.
...
Команда проверяет, совпадает ли наша цена за токен с тем, что считает сам Claude Code: у планов бывают свои ставки, и тогда все суммы смещены в одно и то же число раз. Расхождение она покажет и предложит поправку — применит только с вашего согласия. Если панель заметила модель без цены, команда найдёт её ставки на официальной странице цен Anthropic и тоже запишет только после вашего «да». Если работа списывается кредитами, она же доведёт до сверки с реальным счётом провайдера.

Повторить стоит при смене плана и когда в шапке панели появится модель без цены. Остальное панель проверяет за вас — думать об этом не нужно.

## Ничего не уходит наружу

Панель, сборщик данных и архив не делают ни одного сетевого вызова и не запускают никаких программ. Из библиотек Node импортируются только работа с файлами, путями и домашним каталогом (`node:fs`, `node:path`, `node:os`) — и разбор собственного пути (`node:url`). Ни HTTP, ни сокетов. Панель — локальная страница в VS Code, читающая локальный файл. Единственное, что запускает программу, — сверка ставок из раздела выше, и то по вашей команде. Когда ей нужны ставки новой модели, Claude читает публичную страницу цен Anthropic — это чтение страницы, ваши данные туда не уходят.
...
**Сверка ставок стоит один короткий запрос** — на дешёвой модели это центы, на рабочей дороже, потому что в кэш пишется весь ваш постоянный багаж. Прогоняется она вручную и редко: при установке, при смене плана и когда вышла новая модель — тогда к этому добавляется строка этой модели со страницы цен, это сотни токенов.
